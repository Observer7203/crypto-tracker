...
<div class="bg-white p-6 rounded-lg shadow max-w-xl mx-auto">
    <h1 class="text-xl font-semibold mb-4">Добавить крипто-пару</h1>
    ...
    <form method="POST" action="{{ route('pairs.store') }}" class="space-y-4">
        @csrf

        <div class="relative">
            <label class="block font-medium text-sm text-gray-700">Base Currency</label>
    <select name="base_currency" class="mt-1 block w-full rounded border-gray-300 shadow-sm">
        @foreach ($baseCurrencies as $base)
            <option value="{{ $base }}" {{ old('base_currency') == $base ? 'selected' : '' }}>
                {{ $base }}
            </option>
        @endforeach
    </select>
    ...
    <select name="quote_currency" class="mt-1 block w-full rounded border-gray-300 shadow-sm">
        @foreach ($quoteCurrencies as $quote)
            <option value="{{ $quote }}" {{ old('quote_currency') == $quote ? 'selected' : '' }}>
                {{ $quote }}
            </option>
        @endforeach
    </select>
        ...
        <div>
            ...
            <input type="number" name="update_interval" value="{{ old('update_interval') }}"
